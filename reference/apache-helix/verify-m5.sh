...
SCENARIOS="$ROOT/reference/apache-helix/scenarios/m5"
JAVA_ORACLE="$ROOT/reference/apache-helix/scripts/run-java-oracle.sh"
COMPARATOR="$ROOT/reference/apache-helix/scripts/compare-m5-results.py"
...
[[ -x "$ROOT/reference/apache-helix/verify-m4.sh" ]] || fail "missing prerequisite verifier: reference/apache-helix/verify-m4.sh"
[[ -d "$SCENARIOS" ]] || fail "missing M5 scenarios: $SCENARIOS"

TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/clustodian-m5.XXXXXX")"
trap 'rm -rf "$TEMP_DIR"' EXIT

echo "==> Verifying prerequisite M4" >&2
"$ROOT/reference/apache-helix/verify-m4.sh"
...
if [[ ${#scenarios[@]} -eq 0 ]]; then
  fail "no M5 scenarios found under $SCENARIOS"
fi

echo >&2
echo "==> Apache Helix 2.0.1 MessageSelectionStage differential tests" >&2
...
for scenario in "${scenarios[@]}"; do
  ...
  # Run both implementations twice. The supported M5 semantic result must be
  ...
  if ! python3 "$COMPARATOR" "$java_first" "$java_second"; then
    echo >&2
    echo "verify-m5: Apache Helix oracle was non-deterministic for $name" >&2
    exit 1
  fi

  if ! python3 "$COMPARATOR" "$rust_first" "$rust_second"; then
    echo >&2
    echo "verify-m5: clustodian was non-deterministic for $name" >&2
    exit 1
  fi

  if ! python3 "$COMPARATOR" "$java_first" "$rust_first"; then
    echo >&2
    echo "verify-m5: semantic mismatch for $name" >&2
...
echo >&2
echo "verify-m5: PASS" >&2
echo "  prerequisite: M4 passed" >&2
echo "  differential scenarios: $count" >&2
echo "  determinism: Java and Rust each reproduced every scenario" >&2
echo "  Java authority: Apache Helix 2.0.1 MessageSelectionStage" >&2
echo "  Rust subject: clustodian state-constrained transition selection" >&2
