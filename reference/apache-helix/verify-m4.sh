...
SCENARIOS="$ROOT/reference/apache-helix/scenarios/m4"
JAVA_ORACLE="$ROOT/reference/apache-helix/scripts/run-java-oracle.sh"
COMPARATOR="$ROOT/reference/apache-helix/scripts/compare-m4-results.py"
...
[[ -x "$ROOT/reference/apache-helix/verify-m3.sh" ]] || fail "missing prerequisite verifier: reference/apache-helix/verify-m3.sh"
[[ -d "$SCENARIOS" ]] || fail "missing M4 scenarios: $SCENARIOS"

TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/clustodian-m4.XXXXXX")"
trap 'rm -rf "$TEMP_DIR"' EXIT

echo "==> Verifying prerequisite M3" >&2
"$ROOT/reference/apache-helix/verify-m3.sh"
...
if [[ ${#scenarios[@]} -eq 0 ]]; then
  fail "no M4 scenarios found under $SCENARIOS"
fi

echo >&2
echo "==> Apache Helix 2.0.1 CRUSH differential placement tests" >&2
...
for scenario in "${scenarios[@]}"; do
  ...
  # Run both implementations twice. CRUSH is expected to be deterministic for
  # identical semantic input; make that part of the acceptance gate.
  ...
  if ! python3 "$COMPARATOR" "$java_first" "$java_second"; then
    echo >&2
    echo "verify-m4: Apache Helix oracle was non-deterministic for $name" >&2
    exit 1
  fi

  if ! python3 "$COMPARATOR" "$rust_first" "$rust_second"; then
    echo >&2
    echo "verify-m4: clustodian was non-deterministic for $name" >&2
    exit 1
  fi

  if ! python3 "$COMPARATOR" "$java_first" "$rust_first"; then
    echo >&2
    echo "verify-m4: semantic mismatch for $name" >&2
...
echo >&2
echo "verify-m4: PASS" >&2
echo "  prerequisite: M3 passed" >&2
echo "  differential scenarios: $count" >&2
echo "  determinism: Java and Rust each reproduced every scenario exactly" >&2
echo "  Java authority: Apache Helix 2.0.1 CrushRebalanceStrategy" >&2
echo "  Rust subject: clustodian CRUSH automatic preference-list placement" >&2
